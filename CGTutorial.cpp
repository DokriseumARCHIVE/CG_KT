...
float rotate_finger_buttom = 0.0f;
float rotate_finger_middle = 0.0f;
float rotate_finger_top = 0.0f;
float rotate_finger_horizon = 0.0f;
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	case GLFW_KEY_PAGE_DOWN:
		...
		break;
	case GLFW_KEY_KP_1:
		rotate_finger_buttom += 1.0f;
		break;
	case GLFW_KEY_KP_3:
		rotate_finger_buttom -= 1.0f;
		break;
	case GLFW_KEY_KP_4:
		rotate_finger_middle += 1.0f;
		break;
	case GLFW_KEY_KP_6:
		rotate_finger_middle -= 1.0f;
		break;
	case GLFW_KEY_KP_7:
		rotate_finger_top += 1.0f;
		break;
	case GLFW_KEY_KP_9:
		rotate_finger_top -= 1.0f;
		break;
	case GLFW_KEY_KP_8:
		rotate_finger_horizon += 1.0f;
		break;
	case GLFW_KEY_KP_2:
		rotate_finger_horizon -= 1.0f;
		break;
...
//wichtig fuer die korrekte darstellung der arm- bzw. fingerglieder
void drawFingerLimbSegment(float h)
{

	glm::mat4 Save = Model;

	Model = glm::translate(Model, glm::vec3(0, 0.5 * h, 0));
...
int main(void)
{
	...
	GLFWwindow* window = glfwCreateWindow(1024, // Breite
		768,  // Hoehe
		"Computergrafik - Pflichtteil", // Ueberschrift
		NULL,  // windowed mode
		NULL); // shared window
...
// VAOs sind Container fuer mehrere Buffer, die zusammen gesetzt werden sollen.
	...
	glm::vec4 lightPos = Model * glm::vec4(0.0f, 0.3f, 0.0f, 1.0f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (!rotatedObject)
			Model = glm::rotate(Model, 30.0f, vec3(0, 1, 0));

		if (rotatedObject) {
			Model = glm::rotate(Model, rotate_angle_x, vec3(1, 0, 0));
			Model = glm::rotate(Model, rotate_angle_y, vec3(0, 1, 0));
			Model = glm::rotate(Model, rotate_angle_z, vec3(0, 0, 1));
		}
		...
		Model = glm::rotate(Model, rotate_finger_buttom, vec3(0, 0, 1));
		Model = glm::rotate(Model, rotate_finger_horizon, vec3(1, 0, 0));
		drawFingerLimbSegment(0.6f);
		Model = glm::translate(Model, glm::vec3(0, 0.5, 0));
		Model = glm::rotate(Model, rotate_finger_middle, vec3(0, 0, 1));
		drawFingerLimbSegment(0.4f);
		Model = glm::translate(Model, glm::vec3(0, 0.4, 0));
		Model = glm::rotate(Model, rotate_finger_top, vec3(0, 0, 1));
		drawFingerLimbSegment(0.2f);
